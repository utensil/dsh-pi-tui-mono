...
const awaitImport = () => ({ appendFileSync });
...
export function createPiSessionShim(ctx, agent, sessionId) {
  ...
  const target = { current: { provider: "deepseek", model: DEFAULT_MODEL } };
  ...
  const shim = {
    ...
    setModel(model) {
      const provider = typeof model?.provider === "string" ? model.provider : "deepseek";
    ...
    cycleModel() {
      ...
      target.current = { provider: next.provider, model: next.id };
      model = next.id;
