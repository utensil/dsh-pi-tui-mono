...
export function createPiSessionShim(ctx, agent, sessionId) {
  ...
  const onSessionEvent = (subject, event) => {
    try {
      appendFileSync("/tmp/dsh-events.log",
        `${new Date().toISOString()} subject=${subject === dshSession ? "SELF" : "other"} type=${event?.type}\n`);
    } catch (probeErr) {
      process.stderr.write(`[dsh-pi] probe failed: ${probeErr?.message}\n`);
    }
    ...
    switch (event.type) {
      ...
      case "assistant/chunk": {
        ...
        if (c?.type === "block-start") {
          ...
          if (streamingBlockType === "thinking") {
      ...
      case "tool/call": {
        ...
        toolCalls.set(callId, { name, arguments: args });
      ...
      case "tool/result": {
        ...
        if (callId) {
          ...
          const output = textBlocks(event.data?.message?.content) ?? "";
          emit({
  ...
  const disposeErrors = ctx.on("agent/error", (payload) => {
    try {
      appendFileSync("/tmp/dsh-events.log",
        `${new Date().toISOString()} AGENT_ERROR ${JSON.stringify(payload)?.slice(0, 400)}\n`);
    } catch {}
  });
  const disposeRetry = ctx.on("llm/retry", (payload) => {
    try {
      appendFileSync("/tmp/dsh-events.log",
        `${new Date().toISOString()} LLM_RETRY ${JSON.stringify(payload)?.slice(0, 300)}\n`);
    } catch {}
  });
